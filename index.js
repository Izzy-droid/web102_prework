...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        game.innerHTML = `
            <img src="${games[i].img}" alt="${games[i].name}" style="width: 100%; height: auto;" />
            <h2>${games[i].name}</h2>
            <p>${games[i].description.toLocaleString()}</p>
...
const unfundedGamesStr = numUnfundedGames === 0 ? "all of our games are unfunded" : `${numUnfundedGames} games`;

// create a new DOM element containing the template string and append it to the description container
const description= document.createElement("p");
const all = GAMES_JSON.length;
const Fuunded = raisedCard.innerHTML;
description.innerHTML = `A total of ${Fuunded} has been raised for ${all} games.  Currently, ${unfundedGamesStr} remain unfunded. We need your help to fund these amazing games!`;
